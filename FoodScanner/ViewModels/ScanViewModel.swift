...
final class ScanViewModel {
    ...
    private func resetData() {
        self.recognizedIngredients = []
        self.selectedIngredients = []
        self.finalIngredients = []
        self.nutritionResults = nil
        self.isNutritionEnabled = false
        self.isIngredientSheetPresented = false
        self.selectedImageURL = nil
        print("✅ Reset all previous scan data!")
    }
    ...
    func fetchNutritionData() {
        ...
        Task {
            ...
            do {
                ...
                isIngredientSheetPresented = false
                try await Task.sleep(for: .seconds(0.3))
                showError = true
                print("❌showError set to: \(showError). Error fetching nutrition data: \(error.localizedDescription)")
    ...
    func deleteFromSelectedItems(ingredientID: UUID) {
        ...
        guard selectedIngredients.indices.contains(index) else {
                print("⚠️ Invalid index, skipping removal!")
                return
            }
        selectedIngredients.remove(at: index)
    }
